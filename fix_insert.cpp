  ...
  Fix(lmp, narg, arg)
{
  ...
  // get template
  if(strcmp(arg[iarg++],"distributiontemplate"))  error->all("Fix insert command expecting keyword 'distributiontemplate'");
  int ifix = modify->find_fix(arg[iarg++]);
  if(ifix < 0 || strcmp(modify->fix[ifix]->style,"particledistribution/discrete"))
    error->all("Fix insert command requires you to define a valid ID for a fix of type particledistribution/discrete");
  ...
  {
    hasargs = false;
    if (strcmp(arg[iarg],"maxattempt") == 0) {
      ...
      iarg += 2;
      hasargs = true;
    } else if (strcmp(arg[iarg],"insert_every") == 0) {
      if (iarg+2 > narg) error->all("Illegal fix insert command");
  ...
  first_ins_step = next_reneighbor;
...
void FixInsert::post_create()
{
  ...
  if(ninsert < ninsert_per)
    final_ins_step = update->ntimestep + 1;
  else
    final_ins_step = update->ntimestep + 1 + static_cast<int>(static_cast<double>(ninsert)/ninsert_per *  static_cast<double>(insert_every)); 

  // print statistics
  print_stats_start();

  // last error check, performed after printing stats so user gets more info
  if(ninsert_per <= 1.) error->all("Illegal fix insert command, would insert <=1 particles per insertion step");
}
...
void FixInsert::print_stats_during(int ninsert_this, double mass_inserted_this)
{
  ...
  {
    if (screen)
      fprintf(screen ,"Particle insertion: inserted %d particles (mass %f) at step %d\n - a total of %d particles (mass %f) inserted so far.\n",
	      ninsert_this,mass_inserted_this,step,ninserted,massinserted);

    if (logfile)
      fprintf(logfile,"Particle insertion: inserted %d particles (mass %f) at step %d\n - a total of %d particles (mass %f) inserted so far.\n",
	      ninsert_this,mass_inserted_this,step,ninserted,massinserted);
  }
}
...
void FixInsert::init()
{
    ...
    fix_distribution->random_init_list(ninsert_per);
...
void FixInsert::pre_exchange()
{
  ...
  if (ninserted + ninsert_this > ninsert) ninsert_this = ninsert - ninserted;

  // generate list of insertions
  
  ninsert_this = fix_distribution->randomize_list(ninsert_this,groupbit);
  if (ninserted + ninsert_this > ninsert) ninsert_this = ninsert - ninserted;

  if(ninsert_this == 0)
  {
      error->warning("Particle insertion: Inserting no particle - check particle insertion settings");
      if (insert_every && ninserted < ninsert) next_reneighbor += insert_every;
      return;
  }
  ...
  int ninserted_this,ninserted_spheres_this;
  ...
  int ninserted_spheres_this_local = fix_distribution->insert(ninserted_this);
  ...
  if (atom->tag_enable) {
    atom->tag_extend();
    atom->natoms += ninserted_spheres_this;
    if (atom->map_style) {
      atom->nghost = 0;
      atom->map_init();
      atom->map_set();
