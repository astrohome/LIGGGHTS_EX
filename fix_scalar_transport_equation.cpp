...
FixScalarTransportEquation::FixScalarTransportEquation(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  if (narg < 15) error->all("Illegal fix transportequation/scalar command, not enough arguments");

  if(strcmp(arg[iarg++],"equation_id")) error->all("Fix transportequation/scalar: expecting keyword 'equation_id'");
  equation_id = new char[strlen(arg[iarg])+1];
  strcpy(equation_id,arg[iarg++]);

  if(strcmp(arg[iarg++],"quantity")) error->all("Fix transportequation/scalar: expecting keyword 'quantity'");
  quantity_name = new char[strlen(arg[iarg])+1];
  strcpy(quantity_name,arg[iarg++]);

  if(strcmp(arg[iarg++],"default_value")) error->all("Fix transportequation/scalar: expecting keyword 'default_value'");
  quantity_0 = atof(arg[iarg++]);

  if(strcmp(arg[iarg++],"flux_quantity")) error->all("Fix transportequation/scalar: expecting keyword 'flux_quantity'");
  flux_name = new char[strlen(arg[iarg])+1];
  strcpy(flux_name,arg[iarg++]);

  if(strcmp(arg[iarg++],"source_quantity")) error->all("Fix transportequation/scalar: expecting keyword 'source_quantity'");
  ...
  if(strcmp(arg[iarg++],"capacity_quantity")) error->all("Fix transportequation/scalar: expecting keyword 'capacity_quantity'");
...
{
  if (!atom->rmass_flag) error->all("Please use an atom style that defines per-particle mass for fix transportequation/scalar");
  ...
  if(capacity_flag)
  {
      if(!force->pair_match("gran", 0)) error->all("Please use a granular pair style for fix transportequation/scalar with capacityflag");
