  ...
  FixInsert(lmp, narg, arg)
{
  ...
  while(iarg < narg && hasargs)
  {
    ...
    if (strcmp(arg[iarg],"insertion_face") == 0)
    {
      ...
      if (strncmp(modify->fix[f_i]->style,"mesh/gran",8))
        error->all("Fix insert/stream: The fix belonging to the id you provided is not of type mesh/gran");
      ...
      if(duration < 1 ) error->all("Illegal fix insert/stream command, 'duration' can not be < 1");
      iarg += 2;
      hasargs = true;
...
void FixInsertStream::calc_insertion_properties()
{
    ...
    if(face_style == FACE_MESH)
    {
        ...
        if(vectorMag3D(v_normal) < 1.e-3)
          error->all("Illegal fix insert/stream command, insertion velocity projected on face normal is < 1e-3");
    ...
    if(insert_every == -1 && extrude_length == 0.)
      error->all("Illegal fix insert/stream command, must define either 'insert_every' or 'extrude_length'");
    if(insert_every > -1 && extrude_length > 0.)
      error->all("Illegal fix insert/stream command, must not provide both 'insert_every' and 'extrude_length'");
    if(extrude_length > 0. && duration > 0)
      error->all("Illegal fix insert/stream command, must not provide both 'extrude_length' and 'duration'");
    ...
    if(insert_every == -1)
    {
        ...
        if(insert_every == 0)
          error->all("Illegal fix insert/stream command, insertion velocity too high or extrude_length too low");
    }
    ...
    else
    {
        ...
        else if (duration > insert_every) error->all("Illegal fix insert/stream command, 'duration' > 'insert_every' not allowed");

        extrude_length = static_cast<double>(duration) * dt * vectorMag3D(v_normal);
        if(extrude_length < 3.*max_r_bound())
          error->all("Illegal fix insert/stream command, 'insert_every' or 'vel' is too small");
    }
...
inline int FixInsertStream::overlap(int i)
{
    ...
    if(dist_normal < -(extrude_length+maxrad)) return 0;
...
inline void FixInsertStream::generate_random(double *pos, double rad)
{
    ...
    if(check_ol_flag)
        r = -1.*(random->uniform()*(extrude_length -    rad) + rad);
    else
