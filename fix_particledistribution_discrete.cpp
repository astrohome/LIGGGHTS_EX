...
void FixParticledistributionDiscrete::random_init_list(int ntotal)
{
    ...
    for(int i = 0; i < ntemplates; i++)
    {
        ...
        int parttogen_max_i = static_cast<int>(static_cast<double>(ntotal) * distweight[i] + 1.01);
        templates[i]->init_ptilist(parttogen_max_i);
        n_pti_max += parttogen_max_i;
    }
...
int FixParticledistributionDiscrete::randomize_list(int ntotal,int insert_groupbit)
{
    if(ntotal > n_pti_max)
    {
